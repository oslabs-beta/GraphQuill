...
function findGQ(string: string) {
  ...
  const validatedQueriesArr = queriesArr.map(queryString => checkQueryBrackets(queryString))
...
function validateBracketToken(char: string): char is keyof typeof BracketTokens {
  ...
  for (let i: number = 0; i < queryString.length; i++) {
    ...
    if (queryString[i] in BracketTokens) {
      stack.push(queryString[i]);
    } 
    // if current char is close bracket
      ...
      // double check that popped is defined before doing anything with it that could throw and error if it's undefined
      if (popped !== undefined) {
